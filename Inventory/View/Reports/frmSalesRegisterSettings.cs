...
namespace Inventory
{
    public partial class frmSalesRegisterSettings : Form, IfrmSelectAccClassID, IfrmDateConverter
    {
        ReportPreference m_ReportPreference;
        ArrayList AccClassID = new ArrayList();
        private string Prefix = "";
        private bool IsFromDate = false;
        public frmSalesRegisterSettings()
        {
            InitializeComponent();
        }
        ...
        {
            InventoryBookSettings m_SRS = new InventoryBookSettings();//for sales register Settings
            ...
            {
                m_SRS.partyGroupID = Convert.ToInt32(LiPartyGroupID.ID);
            }
            else
            {
                m_SRS.partyGroupID = null;
            }
            //for single party
            if (rdpartySingle.Checked)
            {
                m_SRS.PartyID = Convert.ToInt32(liPartyID.ID);
            }
            else
            {
                m_SRS.PartyID = null;
            ...
            if (chkDateRange.Checked)
            {
                ...
                m_SRS.FromDate = Date.ToDotNet(txtFromDate.Text);
                m_SRS.ToDate = Date.ToDotNet(txtToDate.Text);
            }
            else
            {
                m_SRS.FromDate = null;
            ...
            if (chkDepot.Checked)
            {
                m_SRS.DepotID = Convert.ToInt32(LiDepotID.ID);
            }
            else
            {
                m_SRS.DepotID = null;
            ...
            {
                m_SRS.ProjectID = Convert.ToInt32(LiProjectID.ID);
            }
            else
            {
                m_SRS.ProjectID = null;
            }
            m_SRS.AccClassID = AccClassID;
            //frmSalesRegister frm = new frmSalesRegister(m_SRS);          
            //frm.Show();


            if (m_SRS.PartyID > 0 )
            {
                frmSaleRegisterPartyWise frms = new frmSaleRegisterPartyWise(m_SRS);
                frms.Show();
            }
            else
              {
                  frmSalesRegister frms = new frmSalesRegister(m_SRS);
                  frms.Show();
              }
        }
        ...
        private void frmSalesRegisterSettings_Load(object sender, EventArgs e)
         {
            LoadMonths();
           ...
           // ListProject(cboProjectName);
            ...
            DataTable dtrpt = m_ReportPreference.GetPreferenceCount(checkuserid, "SALES_REGISTER");
        ...
        private void ChangeReportPreferences()
        {
            ...
            DataTable dt = m_ReportPreference.GetPreferenceInfo(UserID, "SALES_REGISTER");

            //For Accounting Class
            Code = "SRS_ACCOUNTING_CLASS";
