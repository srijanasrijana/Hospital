...
namespace Common
{
    public partial class frmCountry : Form
    {
        ...
        private void btnsave_Click(object sender, EventArgs e)
        {
            ...
            switch (e_mode)
            {
                case EntryMode.NEW:
                    ...
                    break;
                case EntryMode.EDIT:
                    int dt1 = BusinessLogic.Nationality.updateNationality(Convert.ToInt32(txtNationalityID.Text), txtNationalityName.Text);
                    MessageBox.Show("Record saved");
                    cleartextbox();
                    break;
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                int countryId = 0;
                countryId = Convert.ToInt32(txtNationalityID.Text);
                if (!employees.CheckCountry(countryId))
                {
                    Global.Msg("The country you are about to delete has records on Employee's records, so the country record can not be deleted.");
                    return;
                }
                int i = BusinessLogic.Nationality.deletefromNationality(countryId);
