...
namespace BusinessLogic
{
    public class BankReceipt
    {
        ...
        public void Create(int SeriesID, int LedgerID, string VoucherNo, DateTime BankReceipt_Date, string Remarks, DataTable BankReceiptDetails, int[] AccClassID, int ProjectID)
        {
            ...
            if (BankReceiptDetails.Rows.Count == 0)
            {
            ...
            foreach (DataRow row in BankReceiptDetails.Rows)
            ...
            try
            {
                ...
                Global.m_db.ClearParameter();
                Global.m_db.setCommandType(CommandType.StoredProcedure);
                Global.m_db.setCommandText("Acc.spBankReceiptCreate");
                ...
                Global.m_db.AddParameter("@BankReceipt_Date", SqlDbType.DateTime, BankReceipt_Date);
                ...
                for (int i = 0; i < BankReceiptDetails.Rows.Count; i++)
                {
                    DataRow dr = BankReceiptDetails.Rows[i];
                    //Now go for the Detail Inserts
                    Global.m_db.ClearParameter();
                    Global.m_db.setCommandType(CommandType.StoredProcedure);
                    Global.m_db.setCommandText("Acc.spBankReceiptDetailCreate");
                    Global.m_db.AddParameter("@BankReceiptID", SqlDbType.Int, ReturnID.ToString());
                    ...
                    Global.m_db.AddParameter("@ChequeBank", SqlDbType.NVarChar, 50, dr["ChequeBank"].ToString());
                    if ((dr["ChequeDate"] == ""))//if date range is not selected
                    {
                    ...
                    if (paramReturn.Value.ToString() != "SUCCESS")
                    {
                        ...
                        throw new Exception("Unable to create Bank Receipt");
                    }
                    //Also insert the transaction in tbltransaction
                    Global.m_db.ClearParameter();
                    Global.m_db.setCommandType(CommandType.StoredProcedure);
                    Global.m_db.setCommandText("Acc.spTransactCreate");
                    Global.m_db.AddParameter("@Date", SqlDbType.DateTime, BankReceipt_Date);
                    Global.m_db.AddParameter("@LedgerName", SqlDbType.NVarChar, 300, dr["Ledger"].ToString());//Set same for both for time being
                    Global.m_db.AddParameter("@Language", SqlDbType.NVarChar, 30, Language);
                    Global.m_db.AddParameter("@Amount", SqlDbType.Money, Convert.ToDouble(dr["Amount"]));
                    Global.m_db.AddParameter("@DrCr", SqlDbType.NVarChar, 10, "CREDIT");
                    Global.m_db.AddParameter("@VoucherType", SqlDbType.NVarChar, 20, "BANK_RCPT");
        ...
        public void Modify(int BankReceiptID, int SeriesID, int LedgerID, string VoucherNo, DateTime BankReceipt_Date, string Remarks, DataTable BankReceiptDetails, int[] AccClassID, int ProjectID)
        {
            try
            {
                // Global.m_db.InsertUpdateQry("INSERT INTO Account.tblAccountHead(AccountHeadName,Under_Account_ID,Description,BuiltIn) VALUES ('" + AccountHeadName + "','1','" + Description + "','0')");
                ...
                Global.m_db.ClearParameter();
                Global.m_db.setCommandType(CommandType.StoredProcedure);
                Global.m_db.setCommandText("Acc.spBankReceiptModify");
                Global.m_db.AddParameter("@BankReceiptID", SqlDbType.Int, BankReceiptID);
                Global.m_db.AddParameter("@LedgerID", SqlDbType.Int, LedgerID);
                Global.m_db.AddParameter("@SeriesID", SqlDbType.Int, SeriesID);
                Global.m_db.AddParameter("@Voucher_No", SqlDbType.NVarChar, 30, VoucherNo);//Set same for both for time being
                Global.m_db.AddParameter("@BankReceipt_Date", SqlDbType.DateTime, BankReceipt_Date);
                ...
                Global.m_db.InsertUpdateQry("DELETE FROM Acc.tblBankReceiptDetails WHERE BankReceiptID='" + BankReceiptID.ToString() + "'");
                ...
                Global.m_db.InsertUpdateQry("DELETE FROM Acc.tblTransaction WHERE VoucherType='BANK_RCPT' AND RowID='" + BankReceiptID.ToString() + "'");

                //First delete the previous records of Acc.tblTransactionClass according to VoucherType and RowID
                Global.m_db.InsertUpdateQry("DELETE FROM Acc.tblTransactionClass WHERE VoucherType='BANK_RCPT' AND RowID='" + BankReceiptID.ToString() + "'");
                double Amount = 0;
                for (int i = 0; i < BankReceiptDetails.Rows.Count; i++)
                {
                    DataRow dr = BankReceiptDetails.Rows[i];
                    ...
                    Global.m_db.setCommandType(CommandType.StoredProcedure);
                    Global.m_db.setCommandText("Acc.spBankReceiptDetailCreate");
                    Global.m_db.AddParameter("@BankReceiptID", SqlDbType.Int, BankReceiptID.ToString());
                    Global.m_db.AddParameter("@LedgerName", SqlDbType.NVarChar, 300, dr["Ledger"].ToString());//Set same for both for time being
                    Global.m_db.AddParameter("@Language", SqlDbType.NVarChar, 30, Language);
                    Global.m_db.AddParameter("@Amount", SqlDbType.Money, Convert.ToDouble(dr["Amount"]));
                    Amount += Convert.ToDouble(dr["Amount"]);
                    ...
                    Global.m_db.AddParameter("@DrCr", SqlDbType.NVarChar, 10, "CREDIT");
                    Global.m_db.AddParameter("@VoucherType", SqlDbType.NVarChar, 20, "BANK_RCPT");
                    Global.m_db.AddParameter("@RowID", SqlDbType.Int, BankReceiptID);
                    Global.m_db.AddParameter("@ProjectID", SqlDbType.Int, ProjectID);
                    paramReturn = Global.m_db.AddOutputParameter("@Return", SqlDbType.NVarChar, 20);
                    Global.m_db.ProcessParameter();
                    int ReturnTransactID = Convert.ToInt32(paramReturn.Value);
                    ...
                    foreach (int _AccClassID in AccClassID)
                    {
                        ...
                        Global.m_db.AddParameter("@TransactID", SqlDbType.Int, ReturnTransactID.ToString());
                        Global.m_db.AddParameter("@AccClassID", SqlDbType.Int, _AccClassID);//Set same for both for time being
                        Global.m_db.AddParameter("@RowID", SqlDbType.Int, BankReceiptID.ToString());
                        Global.m_db.AddParameter("VoucherType", SqlDbType.NVarChar, 20, "BANK_RCPT");
                        System.Data.SqlClient.SqlParameter paramTransactClassID = Global.m_db.AddOutputParameter("@Return", SqlDbType.NVarChar, 20);
                        Global.m_db.ProcessParameter();
                        if (paramTransactClassID.Value.ToString() == "FAILURE")
        ...
        public static string InsertVoucherRecurring(DataTable dtVoucherRecurring, int voucherID)
        {
            try
            {
                ...
                if (dtVoucherRecurring.Rows.Count > 0)
                {
                    ...
                    dtVoucherRecurring.Rows[0]["VoucherType"] = "BANK_RECEIPT";
                    res = RecurringVoucher.CreateRecurringVoucherSetting(dtVoucherRecurring);
                    if (res == "FAILURE")
                    {
                        Global.m_db.RollBackTransaction();
                        throw new Exception("Unable to create Bank Receipt Recurring settings.");
                    }
                }
                #endregion
        ...
        public static string ModifyVoucherRecurring(DataTable dtVoucherRecurring, int voucherID)
        {
            try
            {
                ...
                if (dtVoucherRecurring.Rows.Count > 0)
                {
                    ...
                    {
                        Global.m_db.RollBackTransaction();
                        throw new Exception("Unable to modify Bank Receipt recurring settings.");
                    }
                }
                #endregion
        ...
        public static string InsertReference(int voucherID, DataTable dtReference)
        {
            try
            {
                ...
                {
                    VoucherReference.CreateReference(dr, voucherID, "BANK_RCPT");
                }

                foreach (DataRow dr in dtReference.Select("[RefID] is not null"))
                {
                    VoucherReference.CreateReferenceVoucher(dr, voucherID, "BANK_RCPT");
                }
        ...
        public bool RemoveBankReceiptEntry(int BankReceiptID)
        {
            try
            {
                Global.m_db.ClearParameter();
                Global.m_db.setCommandType(CommandType.StoredProcedure);
                Global.m_db.setCommandText("Acc.spRemoveBankReceiptEntry");
                Global.m_db.AddParameter("@BankReceiptID", SqlDbType.Int, BankReceiptID);
                //system.Data.SqlClient.SqlParameter objReturn = Global.m_db.AddOutputParameter("@Return", SqlDbType.NVarChar, 20);
                Global.m_db.ProcessParameter();
                return true;
        ...
        /// <returns></returns>
        public DataTable NavigateBankReceiptMaster(int CurrentID, Navigate NavTo)
        {
            Global.m_db.ClearParameter();
            Global.m_db.setCommandType(CommandType.StoredProcedure);
            Global.m_db.setCommandText("Acc.spBankReceiptNavigate");
            ...
            DataTable dtBankReceiptMst = Global.m_db.GetDataTable();
            return dtBankReceiptMst;
        }
        ...
        /// <param name="CurrentID"></param>
        /// <returns></returns>
        public DataTable GetBankReceiptDetail(int MasterID)
        {
            Global.m_db.ClearParameter();
            Global.m_db.setCommandType(CommandType.StoredProcedure);
            Global.m_db.setCommandText("Acc.spGetBankReceiptDetail");
        ...
        public DataTable GetBankReceiptMaster(int BankReceiptID)
        {
            return Global.m_db.SelectQry("SELECT * FROM Acc.tblBankReceiptMaster WHERE BankReceiptID ='" + BankReceiptID + "'", "BankReceiptMaster");
        }

        public int GetSeriesIDFromMasterID(int MasterID)
        {
            object returnID;
            returnID = Global.m_db.GetScalarValue("SELECT SeriesID FROM Acc.tblBankReceiptMaster WHERE BankReceiptID ='" + MasterID + "'");
            return Convert.ToInt32(returnID);
        }

        /// <summary>
        /// Simply Deletes the Bank Receipt from given Journal ID
        /// </summary>
        /// <param name="BankReceiptID"></param>
        public bool Delete(int BankReceiptID)
        {
            ...
            try
            {
                ...
                Global.m_db.InsertUpdateQry("DELETE Acc.tblTransaction WHERE RowID='" + BankReceiptID.ToString() + "' AND VoucherType='BANK_RCPT'");

                //Delete data from BankPaymentMaster Master, BankPaymentMaster Details gets automatically flushed away

                Global.m_db.InsertUpdateQry("DELETE Acc.tblBankReceiptMaster WHERE BankReceiptID='" + BankReceiptID.ToString() + "'");
                return true;
            }
            catch (Exception ex)
            {
        ...
        {
            object returnID;
            returnID = Global.m_db.GetScalarValue("SELECT BankReceiptID FROM Acc.tblBankReceiptMaster WHERE Voucher_No ='" + vouchernumber + "'");
            return Convert.ToInt32(returnID);
        }

        public static int InsertBankREceipt(string BankReceiptXMLString)
        {
            ...
            try
            {
                ...
                Global.m_db.setCommandText("Acc.xmlBankReceiptInsert");
                Global.m_db.setCommandType(CommandType.StoredProcedure);

                Global.m_db.AddParameter("@bankreceipt", SqlDbType.Xml, BankReceiptXMLString);
        ...
        public static string EditBankReceipt(string BankReceiptXMLString)
        {
            ...
            try
            {
                ...
                Global.m_db.setCommandText("Acc.xmlBankReceiptUpdate");
                Global.m_db.setCommandType(CommandType.StoredProcedure);
