...
namespace Inventory
{
    public partial class frmDebitNote : Form,IfrmAddAccClass, IfrmDateConverter, IfrmAccountLedger, ILOVLedger
    {
        ...
        private DataSet.dsDebitNote dsDebitNoteInvoice = new DataSet.dsDebitNote();
        ...
        {
            bool chkUserPermission = UserPermission.ChkUserPermission("DEBIT_NOTE_CREATE_MODIFY");
            ...
            try
            {
                ...
                for (int i = 1; i <= dtDebitNoteDetail.Rows.Count; i++)
                {
                   ...
                   // grdDebitNote[i, 3].Value = Misc.GetDecimalValueByDecimalPlaces(Convert.ToDecimal(drDetail["Amount"])).ToString();
                    grdDebitNote[i, (int)GridColumn.Amount].Value = Convert.ToDecimal(drDetail["Amount"]).ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces));
                    grdDebitNote[i, (int)GridColumn.Remarks].Value = drDetail["Remarks"].ToString();
                    AddRowDebitNote(grdDebitNote.RowsCount);
                    dsDebitNoteInvoice.Tables["tblDebitNoteDetails"].Rows.Add(drDetail["LedgerName"].ToString(),Convert.ToDecimal(drDetail["Amount"]).ToString(Misc.FormatNumber(Global.Comma_Separated,Global.DecimalPlaces)),drDetail["Remarks"].ToString());
