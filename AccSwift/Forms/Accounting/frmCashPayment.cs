...
namespace Inventory
{
    public partial class frmCashPayment : Form, IfrmAddAccClass, IfrmDateConverter, ILOVLedger,IDueDate
    {
        ...
        private DataSet.dsCashPayment dsCashPayment = new DataSet.dsCashPayment();
        ...
        {
            Del = 0, Code_No, Particular_Account_Head, Amount, Current_Balance, Remarks, Ledger_ID, Current_Bal_Actual
        };
        ...
        private void frmCashPayment_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                #region Loading Cash Account as per user setting
                ...
                #endregion          
                cmboCashAccount_SelectedIndexChanged(sender, e);

                grdCashPayment.Redim(2, 8);
                ...
                if (m_CashPaymentID > 0)
                {
                    ...
                    try
                    {
                        ...
                        txtDate.Text = Date.DBToSystem(drCashPaymentMaster["CashPayment_Date"].ToString());
                        txtRemarks.Text = drCashPaymentMaster["Remarks"].ToString();
                        txtCashPaymentID.Text = drCashPaymentMaster["CashPaymentID"].ToString();
                        dsCashPayment.Tables["tblCashPaymentMaster"].Rows.Add(cboSeriesName.Text, drCashPaymentMaster["Voucher_No"].ToString(), Date.DBToSystem(drCashPaymentMaster["CashPayment_Date"].ToString()), cmboCashAccount.Text, drCashPaymentMaster["Remarks"].ToString());
                        ...
                        if (NumberOfFields > 0)
                        {
                            if (NumberOfFields == 1)
                            {
                                ...
                                txtfirst.Text = drCashPaymentMaster["Field1"].ToString();
                                txtsecond.Text = drCashPaymentMaster["Field2"].ToString();
                                txtthird.Text = drCashPaymentMaster["Field3"].ToString();
                                txtfourth.Text = drCashPaymentMaster["Field4"].ToString();
                                txtfifth.Text = drCashPaymentMaster["Field5"].ToString();
                            }
        ...
        {          
            ...
            try
            {
                if (grdCashPayment[CurrRowPos, (int)GridColumn.Particular_Account_Head].Value.ToString() == "(NEW)" || grdCashPayment[CurrRowPos, (int)GridColumn.Particular_Account_Head].Value.ToString() == "")
                    return;
            }
            catch
            {
        ...
        private void Amount_Focus_Lost(object sender, EventArgs e)
        {
            ...
            string AccName = (string)(grdCashPayment[RowCount - 1, (int)GridColumn.Particular_Account_Head].Value);
            //Check if the input value is correct
            if (grdCashPayment[Convert.ToInt32(CurRow), (int)GridColumn.Amount].Value == "" || grdCashPayment[Convert.ToInt32(CurRow), (int)GridColumn.Amount].Value == null)
                grdCashPayment[Convert.ToInt32(CurRow), (int)GridColumn.Amount].Value = (0).ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces));

            if (!Misc.IsNumeric(grdCashPayment[Convert.ToInt32(CurRow), (int)GridColumn.Amount].Value))
            {
            ...
            double checkformat = Convert.ToDouble(grdCashPayment[Convert.ToInt32(CurRow), (int)GridColumn.Amount].Value.ToString());
            string insertvalue = checkformat.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces));
            grdCashPayment[Convert.ToInt32(CurRow), (int)GridColumn.Amount].Value = insertvalue;
            if (AccName != "(NEW)")
            {
                AddRowCashPayment(RowCount);
            }
        ...
        {           
            ...
            try
            {
                TempAmount = Convert.ToDecimal(grdCashPayment[Convert.ToInt32(RowPosition), (int)GridColumn.Amount].Value);
            }
            catch
            {
                TempAmount = 0;
            ...
            {
                grdCashPayment[Convert.ToInt32(RowPosition), (int)GridColumn.Current_Balance].Value = (Convert.ToDecimal(TempAmount)).ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)) + " (Cr.)";               
            }
            else
            {
                grdCashPayment[RowPosition, (int)GridColumn.Current_Balance].Value = CurrBal;
            }
            grdCashPayment[RowPosition, (int)GridColumn.Ledger_ID].Value = LdrID;
            grdCashPayment[RowPosition, (int)GridColumn.Current_Bal_Actual].Value = CurrBal;
        ...
        {          
            ...
            if (CurrBal == "")
            {
                CurrentLedgerBalance = grdCashPayment[Convert.ToInt32(RowPosition), (int)GridColumn.Current_Bal_Actual].ToString();
                CurrentBalance = grdCashPayment[Convert.ToInt32(RowPosition), (int)GridColumn.Current_Bal_Actual].ToString().Split('(');
            }
            else
            {
        ...
        {
            grdCashPayment[0, (int)GridColumn.Del] = new MyHeader("Del");
            grdCashPayment[0, (int)GridColumn.Del].Column.Width = 40;

            grdCashPayment[0, (int)GridColumn.Code_No] = new MyHeader("Code No.");
            grdCashPayment[0, (int)GridColumn.Code_No].Column.Width = 60;

            grdCashPayment[0, (int)GridColumn.Particular_Account_Head] = new MyHeader("Particular/Accounting Head");
            grdCashPayment[0, (int)GridColumn.Particular_Account_Head].Column.Width = 150;

            grdCashPayment[0, (int)GridColumn.Amount] = new MyHeader("Amount");
            grdCashPayment[0, (int)GridColumn.Amount].Column.Width = 100;

            grdCashPayment[0, (int)GridColumn.Current_Balance] = new MyHeader("Current Balance");
            grdCashPayment[0, (int)GridColumn.Current_Balance].Column.Width = 100;

            grdCashPayment[0, (int)GridColumn.Remarks] = new MyHeader("Remarks");
            grdCashPayment[0, (int)GridColumn.Remarks].Column.Width = 150;

            grdCashPayment[0, (int)GridColumn.Ledger_ID] = new MyHeader("Ledger ID");
            grdCashPayment[0, (int)GridColumn.Ledger_ID].Column.Visible = false;

            grdCashPayment[0, (int)GridColumn.Current_Bal_Actual] = new MyHeader("Current Balance");
            grdCashPayment[0, (int)GridColumn.Current_Bal_Actual].Column.Visible = false;
        }

        /// <summary>
        /// Adds the row in the Journal field
        /// </summary>
        private void AddRowCashPayment(int RowCount)
        {
            //Add a new row
            ...
            grdCashPayment[i, (int)GridColumn.Del] = btnDelete;

            grdCashPayment[i, (int)GridColumn.Del].AddController(evtDelete);

            grdCashPayment[i, (int)GridColumn.Code_No] = new SourceGrid.Cells.Cell(i.ToString());
            ...
            grdCashPayment[i, (int)GridColumn.Particular_Account_Head] = new SourceGrid.Cells.Cell("", txtAccount);
            ...
            grdCashPayment[i, (int)GridColumn.Particular_Account_Head].AddController(evtAccountFocusLost);
            grdCashPayment[i,  (int)GridColumn.Particular_Account_Head].Value = "(NEW)"; 
            ...
            grdCashPayment[i, (int)GridColumn.Amount] = new SourceGrid.Cells.Cell("", txtAmount);
            txtAmount.Control.TextChanged += new EventHandler(Text_Change);
            grdCashPayment[i, (int)GridColumn.Amount].AddController(evtAmountFocusLost);
            grdCashPayment[i, (int)GridColumn.Amount].Value = "";
            ...
            grdCashPayment[i, (int)GridColumn.Current_Balance] = new SourceGrid.Cells.Cell("", txtCurrentBalance);
            grdCashPayment[i, (int)GridColumn.Current_Balance].Value = "";
            ...
            grdCashPayment[i, (int)GridColumn.Remarks] = new SourceGrid.Cells.Cell("", txtRemarks);
            grdCashPayment[i, (int)GridColumn.Remarks].Value = "";
            ...
            grdCashPayment[i, (int)GridColumn.Ledger_ID] = new SourceGrid.Cells.Cell("", txtLedgerID);
            grdCashPayment[i, (int)GridColumn.Ledger_ID].Value = "";
            ...
            grdCashPayment[i, (int)GridColumn.Current_Bal_Actual] = new SourceGrid.Cells.Cell("", txtCurrBal);
            grdCashPayment[i, (int)GridColumn.Current_Bal_Actual].Value = "";
        ...
        {
            txtVchNo.Enabled = txtDate.Enabled = txtRemarks.Enabled = grdCashPayment.Enabled =cboSeriesName.Enabled=cmboCashAccount.Enabled= cboProjectName.Enabled=tabControl1.Enabled= Enable;
        }
        ...
        {
            bool chkUserPermission = UserPermission.ChkUserPermission("CASH_PAYMENT_CREATE_MODIFY");
            ...
            switch (m_mode)
            {
                ...
                case EntryMode.NEW: //if new button is pressed
                    try
                    {
                        ...
                        NewGrid = " ";
                        OldGrid = " ";
                        NewGrid = NewGrid + "Voucher No" + txtVchNo.Text + "Series" + cboSeriesName.Text + "Project" + cboProjectName.Text + "Date" + txtDate.Text + "Cash A/C" + cmboCashAccount.Text;
                        ...
                        {
                            string particular = grdCashPayment[i + 1, (int)GridColumn.Particular_Account_Head].Value.ToString();
                            string amt = grdCashPayment[i + 1, (int)GridColumn.Amount].Value.ToString();
                            NewGrid = NewGrid + string.Concat(particular, amt);
                        }
                        NewGrid = "NewGridValues" + NewGrid;

                        //Call to Convert into XML Format
                        string CashPaymentXMLString = ReadAllJournalEntry();
                        ...
                        for (int i = 0; i < grdCashPayment.Rows.Count - 2; i++) //Skip the first row(being header) and last row(being (NEW))
                        {
                          //  string[] ledgerName = grdCashReceipt[i + 1, 2].ToString().Split('[');
                            string[] ledgerName = grdCashPayment[i + 1, (int)GridColumn.Particular_Account_Head].ToString().Split('[');
                            CashPaymentDetails.Rows.Add(ledgerName[0].ToString(), grdCashPayment[i + 1, (int)GridColumn.Amount].Value, grdCashPayment[i + 1, (int)GridColumn.Current_Balance].Value);

                            //Check for empty Amount
                            object objAmount = grdCashPayment[i + 1, (int)GridColumn.Amount].Value;
                            if (objAmount == null)
                            {
                            ...
                            try
                            {
                                totalgrdAmount += Convert.ToDouble(grdCashPayment[i + 1, (int)GridColumn.Amount].Value);
                            }
                            catch
                            {
        ...
        private string ReadAllJournalEntry()
        {
            ...
            tw.WriteStartElement("CASHPAYMENT");
            {
                ...
                string CashPaymentID = System.Convert.ToString(txtCashPaymentID.Text);
                ...
                for (int i = 0; i < OnlyReqdDetailRows; i++)
                {
                    ...
                    LedgerID = System.Convert.ToInt32(grdCashPayment[i + 1, (int)GridColumn.Ledger_ID].Value);
                    Amount = System.Convert.ToDecimal(grdCashPayment[i + 1, (int)GridColumn.Amount].Value);
                    RemarksDetail = System.Convert.ToString(grdCashPayment[i + 1, (int)GridColumn.Remarks].Value);
        ...
        private bool ValidateGrid()
        {
            ...
            for (int i = 0; i < grdCashPayment.Rows.Count - 1; i++)
            {
                try
                {
                    ...
                    try
                    {
                        tempValue = System.Convert.ToInt32(grdCashPayment[i + 1, (int)GridColumn.Ledger_ID].Value);
                    }
                    catch (Exception ex)
                    {
                        tempValue = 0;
                    ...
                    try
                    {
                        tempDecValue = System.Convert.ToDecimal(grdCashPayment[i + 1, (int)GridColumn.Amount].Value);
                    }
                    catch (Exception ex)
                    {
                        tempDecValue = 0;
                    ...
                    {
                        if (i + 2 == grdCashPayment.Rows.Count && grdCashPayment[i + 1, (int)GridColumn.Particular_Account_Head].Value.ToString() == "(NEW)")
                        {
                            //Do Nothing
                        }
                        else
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            ...
            isNew = false;
            OldGrid = " ";
            OldGrid = OldGrid + "Voucher No" + txtVchNo.Text + "Series" + cboSeriesName.Text + "Project" + cboProjectName.Text + "Date" + txtDate.Text + "Cash A/C" + cmboCashAccount.Text;
            //Collect the Contents of the grid for audit log
            for (int i = 0; i < grdCashPayment.Rows.Count - 2; i++)//ignore the first row being header and last row being new
            {
                string particular = grdCashPayment[i + 1, (int)GridColumn.Particular_Account_Head].Value.ToString();
            ...
            //    return;
            //}
            if (txtCashPaymentID.Text.Length <= 0)
            {
        ...
        private bool Navigation(Navigate NavTo)
        {
            try
            {
                ...
                try
                {
                    VouchID = Convert.ToInt32(txtCashPaymentID.Text);
                ...
                if (dtProjectInfo.Rows.Count != 0)
                {
                    ...
                    cboProjectName.Text = drProjectInfo["Name"].ToString();
                    txtCashPaymentID.Text = drCashPaymentMaster["CashPaymentID"].ToString();
                    dsCashPayment.Tables["tblCashPaymentMaster"].Rows.Add(cboSeriesName.Text, drCashPaymentMaster["Voucher_No"].ToString(), Date.DBToSystem(drCashPaymentMaster["CashPayment_Date"].ToString()), cmboCashAccount.Text, drCashPaymentMaster["Remarks"].ToString(), drProjectInfo["Name"].ToString());
                }
                else
                {
                    cboProjectName.Text = "None";
                    txtCashPaymentID.Text = drCashPaymentMaster["CashPaymentID"].ToString();
                    dsCashPayment.Tables["tblCashPaymentMaster"].Rows.Add(cboSeriesName.Text, drCashPaymentMaster["Voucher_No"].ToString(), Date.DBToSystem(drCashPaymentMaster["CashPayment_Date"].ToString()), cmboCashAccount.Text, drCashPaymentMaster["Remarks"].ToString(),"None");
                }
                //For Additional Fields
                if (NumberOfFields > 0)
                {
                 ...
                 DataTable dtCashPaymentDetail = m_CashPayment.GetCashPaymentDetail(Convert.ToInt32(txtCashPaymentID.Text));
                for (int i = 1; i <= dtCashPaymentDetail.Rows.Count; i++)
                {
                    DataRow drDetail = dtCashPaymentDetail.Rows[i - 1];
                    grdCashPayment[i, (int)GridColumn.Code_No].Value = i.ToString();
                    grdCashPayment[i, (int)GridColumn.Particular_Account_Head].Value = drDetail["LedgerName"].ToString();
                    //grdCashPayment[i, 3].Value = drDetail["LedgerFolioNo"].ToString();
                    //grdCashPayment[i, 4].Value = drDetail["BudgetHeadNo"].ToString();                    
                    grdCashPayment[i, (int)GridColumn.Amount].Value = Misc.GetDecimalValueByDecimalPlaces(Convert.ToDecimal(drDetail["Amount"])).ToString();
                    grdCashPayment[i, (int)GridColumn.Remarks].Value = drDetail["Remarks"].ToString();
                    grdCashPayment[i, (int)GridColumn.Ledger_ID].Value = drDetail["LedgerID"].ToString();
                    ...
                    {
                        grdCashPayment[i, (int)GridColumn.Current_Balance].Value = (0).ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces));
                    ...
                    else
                    {
                        ...
                        */
                        ...
                        grdCashPayment[i, (int)GridColumn.Current_Balance].Value = strBalance;
                        grdCashPayment[i, (int)GridColumn.Current_Bal_Actual].Value = Balance.ToString();

                    }
                    AddRowCashPayment(grdCashPayment.RowsCount);
                    dsCashPayment.Tables["tblCashPaymentDetails"].Rows.Add(drDetail["LedgerName"].ToString(), Misc.GetDecimalValueByDecimalPlaces(Convert.ToDecimal(drDetail["Amount"])).ToString(), drDetail["Remarks"].ToString());
                ...
                DataTable dtAccClassDtl = AccountClass.GetAccClassInfo(Convert.ToInt32(txtCashPaymentID.Text), "CASH_PMNT");
        ...
        {
            CashPaymentIDCopy = Convert.ToInt32(txtCashPaymentID.Text);
        }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (Global.MsgQuest("Are you sure you want to delete the cash payment - " + txtCashPaymentID.Text + "?") == DialogResult.Yes)
                {
                    CashPayment DelCashReceipt = new CashPayment();
                    if (DelCashReceipt.RemoveCashPaymentEntry(Convert.ToInt32(txtCashPaymentID.Text)))
                    {
                        Global.Msg("Cash Payment -" + txtCashPaymentID.Text + " deleted successfully!");
                        ...
                        Global.MsgError("There was an error while deleting cash payment -" + txtCashPaymentID.Text + "!");
                }
            }
            catch (Exception ex)
            {
        ...
        private void btn_groupvoucherposting_Click(object sender, EventArgs e)
        {
            ...
            int rowid = cpayment.GetRowID(txtVchNo.Text);
            int SID = System.Convert.ToInt32(SeriesID.ID);
            int ProjectID = System.Convert.ToInt32(liProjectID.ID);
            ...
            {
                dtbulkvoucher.Rows.Add(grdCashPayment[i + 1, (int)GridColumn.Particular_Account_Head].Value, "Debit", grdCashPayment[i + 1, (int)GridColumn.Amount].Value, grdCashPayment[i + 1, (int)GridColumn.Ledger_ID].Value, "CASH_PMNT", txtVchNo.Text, grdCashPayment[i + 1, (int)GridColumn.Remarks].Value);
            }
