    ...
    public partial class frmAccountSetup : Form, ILOVCurrency, IfrmOpeningBalance, IfrmPreviousYearBalance
    {
        ...
        private void frmAccountSetup_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                ShowAccountHeadInTreeView(tvAccount, null, 0);
                ...
                AccountGroup.Search(SearchIn.Account_Groups, SearchOp.Begins_With, "", SearchOp.Begins_With, "", LangMgr.DefaultLanguage);//Search a blank text with begins with so that all the available data is listed
        ...
        public void ShowAccountHeadInTreeView(TreeView tv, TreeNode n, int Group_ID)
        {
        ...
        {
            isNew = true;
            bool chkUserPermission = UserPermission.ChkUserPermission("ACCOUNT_CREATE_MODIFY");
        ...
        {
            txtGroupCode.Enabled = txtGroupName.Enabled = cboParentGrp.Enabled = txtDescription.Enabled = Enable;
        ...
        private void btnLdrSave_Click(object sender, EventArgs e)
        {
            ...
            if (chkUserPermission == false)
            {
                Global.MsgError("Sorry! you don't have permission to Create. Please contact your administrator for permission.");
                return;
            }
            //Check Validation
            if (!ValidateLedger())
                    ...
                    if (tabAccountSetup.SelectedIndex == 1) //Ledger Selected
                    {
                        try
                        {
                           ...
                           // int LdrID = Ledger.GetLedgerIdFromName(ledgername, LangMgr.DefaultLanguage);
                            OpeningBalance.InsertAccountOpeningBalance(LdrID, FromOpeningBalance);
                ...
                case EntryMode.EDIT:
                    if (tabAccountSetup.SelectedIndex == 1) //Ledger Selected
                    {
                        ...
                        try
                        {
                            ...
                            bool result = acLedger.Modify(Convert.ToInt32(txtLdrLedgerID.Text), txtLedgerCode.Text.Trim(), txtLedgerName.Text.Trim(), lstLdrAcGrp.ID, 0.00, strPrevYrBal, lstOpCCY.ID, CCYRate, dtOpBalCCR, txtLdrRemarks.Text.Trim(), txtLdrPerName.Text.Trim(), txtLdrAddress1.Text.Trim(), txtLdrAddress2.Text.Trim(), txtLdrCity.Text.Trim(), txtLdrTelephone.Text.Trim(), txtLdrEmail.Text.Trim(), txtLdrCompany.Text.Trim(), txtLdrWebsite.Text.Trim(), txtVatPanNo.Text.Trim(), Convert.ToDouble(txtCreditLimit.Text), Convert.ToBoolean(chkAutoCalculate.Checked ? 1 : 0), RateTextBox.Text.Length > 0 ? float.Parse(RateTextBox.Text) : 0,OldLedgerDesc,NewLedgerDesc,isNew);

                            //Get current LedgerID and save it to tblOpeningBalance
                            //int LdrID = Ledger.GetLedgerIdFromName(txtLedgerName.Text.Trim(), LangMgr.Language);
                            if (FromOpeningBalance.Rows.Count > 0)
                            {
                                OpeningBalance.InsertAccountOpeningBalance(Convert.ToInt32(txtLdrLedgerID.Text), FromOpeningBalance);
                            }
        ...
        public void AddOpeningBalance(DataTable AllOpeningBalance)
        {
            ...
            switch (m_SearchIn)
            {
                ...
                case SearchIn.Accounts_Under:
                    try
                    {
                        ...
                        DataTable dtSrchGroup = AccountGroup.Search(m_SearchIn, SrchOP1, SearchParam1, SrchOP2, SearchParam2, LangMgr.DefaultLanguage);
                ...
                case SearchIn.Ledgers:
                    ...
                    try
                    {
                        ...
                        DataTable dtSrchLedger = Ledger.Search(m_SearchIn, SrchOP1, SearchParam1, SrchOP2, SearchParam2, LangMgr.DefaultLanguage);
        ...
        {
            if (tabDisplay.SelectedIndex == 0)//TreeView Selected
            {
                //fill the account heads again
                tvAccount.Nodes.Clear();
                ShowAccountHeadInTreeView(tvAccount, null, 0);
            }
            else if (tabDisplay.SelectedIndex == 1)//ListView Selected
            {
                //fill the account heads again
                //Search(SearchIn.Account_Groups, SearchOp.Begins_With, "", SearchOp.Begins_With, "");
            }
