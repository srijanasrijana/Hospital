...
/****** Object:  StoredProcedure [Acc].[spGetLedgerNameNOpBal]    Script Date: 1/12/2017 11:39:22 AM ******/
SET ANSI_NULLS ON
GO
...
END
 ...
 set @SqlStmt='Select Acc.tblLedger.LedgerID, EngName as LedgerName,ISNULL(OpenBal,0),isnull(OpenBalDrCr,'' '') as OpenBalDrCr,
			case OpenBalDate 
			when NULL then NULL else Date.fnEngtoNep(OpenBalDate )
			END,Acc.tblLedger.LedgerCode
...
--set @SqlStmt=@SqlStmt+' where Acc.tblLedger.LedgerID in(select *from' +CONVERT(varchar,@tblLedgerID)+ ')'
 Select Acc.tblLedger.LedgerID, EngName as LedgerName,ISNULL(OpenBal,0) as OpenBal,isnull(OpenBalDrCr,' ') as OpenBalDrCr,
			case OpenBalDate 
		when NULL then NULL else Date.fnEngtoNep(OpenBalDate )			END as OpenBalDate,LedgerCode		from Acc.tblLedger left outer join  Acc.tblOpeningBalance on 
