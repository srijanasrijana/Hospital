...
/****** Object:  StoredProcedure [Acc].[spGetDebtorAging]    Script Date: 4/5/2017 11:03:57 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE  PROCEDURE [Acc].[spGetDebtorAging]
(
...
Insert into @tblDetails
	...
	CLOSE db_Maincursor
	 ...
	 select ds.LedgerID,l.EngName,isnull((sum(DebitTotal+OpenBalDr)-sum(CreditTotal+OpenBalCr)),0) from @tblDetails ds inner join Acc.tblLedger l on ds.LedgerID=l.LedgerID   group by ds.LedgerID,l.EngName having (isnull((sum(DebitTotal+OpenBalDr)-sum(CreditTotal+OpenBalCr)),0))>0
			 			   ...
			 			   select  f.LedgerID,l.LedgerName,l.TotalAmount,f.DueDate,f.OverDueDay,f.Amount,f.VoucherType,f.RowID from @tblFinal as f inner join @tblldgFinal l on f.LedgerID=l.LedgerID
