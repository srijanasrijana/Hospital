...
/****** Object:  StoredProcedure [Acc].[spGetGroupNldgBal]    Script Date: 2/6/2017 9:39:10 AM ******/
SET ANSI_NULLS ON
GO
...
Create  PROCEDURE [Acc].[spGetGroupNldgBal]
(
...
GroupID int,DebitTotal decimal(19,5),CreditTotal decimal(19,5),OpenBalDr decimal(19,5),OpenBalCr decimal(19,5))
	...
	SET @SqlStmt='SELECT l.LedgerID AS LedgerID,isNULL(SUM(t.Debit_Amount),0) debit,isNULL(SUM(t.credit_amount),0) credit, isnull(sum(o.Debit)/count(*),0) as OpenBalDr, isnull(sum(o.Credit)/count(*),0) as OpenBalCr from acc.tblLedger as l left outer join acc.tblTransaction as t on l.LedgerID=t.ledgerID '+@AccClassSTMT+' LEFT JOIN  #tmpOpenBal o on l.LedgerID=o.LedgerID   group by l.LedgerID'; 
...
declare @ID int=0
declare @DebitAmount DECIMAL(19,5)
declare @CreditAmount DECIMAL(19,5)
declare @DrOpenBal DECIMAL(19,5)
declare @CrOpenBal DECIMAL(19,5)
declare @str nvarchar(max)='';


--get all group from tblGroup
declare @tblGroup as table(groupID int) 

if(@GroupID!='' and @GroupID!=0)
begin
     insert into @tblGroup values(@GroupID)
	 
	 ;with CtrRequiredGrpID(RgpID)
	 as(select GroupID from acc.tblGroup where Parent_GrpID=@GroupID
	 union all 
	 select acc.tblGroup.GroupID from acc.tblGroup,CtrRequiredGrpID where Parent_GrpID=CtrRequiredGrpID.RgpID)  

	 insert into @tblGroup
	 select *from CtrRequiredGrpID
end
else
begin
     insert into @tblGroup
     select GroupID from Acc.tblGroup
end

declare @ref as table(RefGID int)
insert into @ref select *from @tblGroup

declare @tempGroupID as table(grpID int)

declare  @finalTable as table(AccountName nvarchar(50),AccountCode nvarchar(50),AccountID int,ParentGroupID int,DebitTotal decimal(19,5),CreditTotal  decimal(19,5),OpenBalDr  decimal(19,5),OpenBalCr  decimal(19,5),	AccountType nvarchar(20))
...
select EngName,LedgerCode,GroupID,isnull(Parent_GrpID,0),@DebitAmount,@CreditAmount,@DrOpenBal,@CrOpenBal,'GROUP' from Acc.tblGroup where GroupID=@ID
 ...
 insert into @finalTable
 select LedgerName,LedgerCode,LedgerID,GroupID,DebitTotal,CreditTotal,OpenBalDr,OpenBalCr,'LEDGER' from @LedgerDetails where GroupID in(select *from @ref)
