...
/****** Object:  StoredProcedure [Acc].[spGetLedgerDetailsByGroupOrLedgerID]    Script Date: 2017-07-17 1:24:44 PM ******/
SET ANSI_NULLS ON
GO
...
AS
	...
	SET @SqlStmt='SELECT ' + @SelectCol + 
		',  ABS((isnull(t.Debit,0) - convert( decimal(20,2),isnull(t.Credit,0) + isnull(t.OpenBalDr,0) -isnull(t.OpenBalCr,0))))  FinalBal, CAST(
             CASE 
                  WHEN (isnull(t.Debit,0) - isnull(t.Credit,0) + isnull(t.OpenBalDr,0) -isnull(t.OpenBalCr,0)) < 0
                    THEN ''CREDIT'' 
                  ELSE ''DEBIT'' 
             END AS nvarchar) as DrCr, l.DrCr as LedgerType FROM acc.tblLedger l LEFT OUTER JOIN #temp t ON t.LedgerID = l.LedgerID
