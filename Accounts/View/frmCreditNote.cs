...
namespace Accounts
{
    public partial class frmCreditNote : Form,IfrmAddAccClass, IfrmDateConverter, IfrmAccountLedger, ILOVLedger
    {
        ...
        private void AddGridHeader()
        {
            ...
            grdCreditNote.Columns[(int)GridColumn.Del].AutoSizeMode = SourceGrid.AutoSizeMode.EnableAutoSize;

            grdCreditNote[0, (int)GridColumn.SNo] = new SourceGrid.Cells.ColumnHeader("S.No.");
            grdCreditNote.Columns[(int)GridColumn.SNo].AutoSizeMode = SourceGrid.AutoSizeMode.EnableAutoSize;
            ...
            grdCreditNote.Columns[(int)GridColumn.Account].AutoSizeMode = SourceGrid.AutoSizeMode.EnableAutoSize;
            ...
            grdCreditNote.Columns[(int)GridColumn.Amount].AutoSizeMode = SourceGrid.AutoSizeMode.EnableAutoSize;
            ...
            grdCreditNote.Columns[(int)GridColumn.Remarks].AutoSizeMode = SourceGrid.AutoSizeMode.EnableAutoSize;
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (m_mode == EntryMode.NEW)
                chkUserPermission = UserPermission.ChkUserPermission("CREDIT_NOTE_CREATE");

            else if (m_mode == EntryMode.EDIT)
                chkUserPermission = UserPermission.ChkUserPermission("CREDIT_NOTE_MODIFY");
        ...
        {
            bool chkUserPermission = UserPermission.ChkUserPermission("CREDIT_NOTE_MODIFY");
            if (chkUserPermission == false)
            {
        ...
        private bool Navigation(Navigate NavTo)
        {
            try
            {
                ...
                for (int i = 1; i <= dtCreditNoteDetail.Rows.Count; i++)
                {
                    ...
                    grdCreditNote[i, 1].Value = i.ToString();
                    grdCreditNote[i, 2].Value = drDetail["LedgerName"].ToString();
                    grdCreditNote[i, 3].Value =(Convert.ToDecimal(drDetail["Amount"])).ToString();
                    grdCreditNote[i, 4].Value = drDetail["Remarks"].ToString();
                    AddRowCreditNote(grdCreditNote.RowsCount);
                    dsCreditNote.Tables["tblCreditNoteDetails"].Rows.Add(drDetail["LedgerName"].ToString(), drDetail["Amount"].ToString(), drDetail["Remarks"].ToString());
        ...
        private void btnNew_Click(object sender, EventArgs e)
        {
            ...
            bool chkUserPermission = UserPermission.ChkUserPermission("CREDIT_NOTE_CREATE");
            if (chkUserPermission == false)
            {
