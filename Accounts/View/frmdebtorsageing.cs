...
namespace Accounts
{
    public partial class frmDebtorAgeing : Form, IfrmDateConverter, IfrmSelectAccClassID
    {
        BusinessLogic.AccountGroup acc = new AccountGroup();
        private string Prefix = "";
        ArrayList AccClassID = new ArrayList();
        public frmDebtorAgeing()
        {
            InitializeComponent();
        }
        IMDIMainForm frmMain;
        public frmDebtorAgeing(IMDIMainForm frm)
        {
            InitializeComponent();
            frmMain = frm;
        }
        public frmDebtorAgeing(DebtorsSettings ds)
        {
           // m_DS = ds;
            InitializeComponent();
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (!(UserValidation.validateInt(txtfirstperiod.Text.Trim())) || !(UserValidation.validateInt(txtsecondperiod.Text.Trim())) || !(UserValidation.validateInt(txtthirdperiod.Text.Trim())) || !(UserValidation.validateInt(txtfourthperiod.Text.Trim())))
                {
                    Global.MsgError("Aging periods must be in valid integer numbers");
                    return;
                }

                if (Convert.ToInt32(txtfirstperiod.Text.Trim()) >= Convert.ToInt32(txtsecondperiod.Text.Trim()) || Convert.ToInt32(txtsecondperiod.Text.Trim()) >= Convert.ToInt32(txtthirdperiod.Text.Trim()) || Convert.ToInt32(txtthirdperiod.Text.Trim()) >= Convert.ToInt32(txtfourthperiod.Text.Trim()))
                {
                    Global.MsgError("Successive period numbers must be greater than the precedings");
                    return;
                }
                ...
                frmdebtorsageing da = new frmdebtorsageing(m_DSShow,frmMain);
                da.ShowDialog();
            }
            catch (Exception ex)
            {
