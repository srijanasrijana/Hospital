...
namespace Accounts
{
    public partial class frmchequereceipt : Form, IfrmDateConverter, ILOVLedger, IfrmAddAccClass
    {
        ...
        private Accounts.Model.dsChequeReceipt dsChequeReceipt = new Accounts.Model.dsChequeReceipt();
        ...
        public frmchequereceipt(int ChequeReceiptID, bool isRecurring)
        {
            InitializeComponent();
        ...
        private void Amount_Focus_Lost(object sender, EventArgs e)
        {
            ...
            grdchequereceipt[Convert.ToInt32(CurRow), (int)GridColumn.Amount].Value = checkformat;
        ...
        private bool Navigation(Navigate NavTo)
        {
            try
            {
                ...
                for (int i = 1; i <= dtChequeReceiptDetail.Rows.Count; i++)
                {
                    ...
                    grdchequereceipt[i, (int)GridColumn.Amount].Value = (Convert.ToDecimal(drDetail["Amount"])).ToString();
                    ...
                    dsChequeReceipt.Tables["tblChequeReceiptDetail"].Rows.Add(drDetail["LedgerName"].ToString(),(Convert.ToDecimal(drDetail["Amount"])).ToString(), drDetail["ChequeNumber"].ToString(), drDetail["ChequeBank"].ToString(), Date.DBToSystem(drDetail["chequeDate"].ToString()), drDetail["Remarks"].ToString());
