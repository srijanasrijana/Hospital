...
namespace Accounts
{
    ...
    {
        private Accounts.Model.dsCashFlow dsCashFLow = new Model.dsCashFlow();
       ...
       // ArrayList ProjectIDs = new ArrayList();
        ...
        IMDIMainForm m_mainMDI;
        public frmCashFlow(CashFlowSettings CashFlow,IMDIMainForm frm)
        {
            InitializeComponent();
            m_mainMDI = frm;
            ...
            m_CashFlow.ShowLedger = CashFlow.ShowLedger;
        ...
        private void CreateColumnsforTempTable()
        {
            ...
            dtTemp.Columns.Add("AmountInflow", typeof(string));
            dtTemp.Columns.Add("AmountOutflow", typeof(string));
            dtTemp.Columns.Add("Type", typeof(string));
            dtTemp.Columns.Add("RowNumber",typeof(string));
        }
        ...
        private void FillGridWithData()
        {
            try
            {
                ...
                dgCashFlow.Columns.Clear(); // first clear all columns to reload the data in dgDayBook
                dgCashFlow.FixedRows = 1;
                DevAge.ComponentModel.IBoundList bd = new DevAge.ComponentModel.BoundDataView(mView);
                CreateDataGridColumns(dgCashFlow.Columns, bd);
                dgCashFlow.DataSource = bd;
                dgCashFlow.Width = dgCashFlow.Width - 5;
                dgCashFlow.Width = this.Width - 25;
        ...
        private void CreateDataGridColumns(SourceGrid.DataGridColumns columns, DevAge.ComponentModel.IBoundList boundList)
        {
            ...
            SourceGrid.Cells.Views.Cell backGroundColor = new SourceGrid.Cells.Views.Cell();
            backGroundColor.BackColor = Global.Grid_Color;

            SourceGrid.Conditions.ConditionView con_backGroundView = new SourceGrid.Conditions.ConditionView(backGroundColor);
            con_backGroundView.EvaluateFunction = delegate(SourceGrid.DataGridColumn column, int gridRow, object itemRow)
            {
                try
                {
                    DataRowView row = (DataRowView)itemRow;
                  //  MessageBox.Show(row["RowNumber"].ToString());

                    if (row["Type"].ToString() == "LEDGER")
                    {
                        backGroundColor.ForeColor = System.Drawing.Color.Blue;
                        backGroundColor.Font = new Font(LangMgr.GetFont(), FontStyle.Regular);
                    }
                    else
                    {
                        backGroundColor.ForeColor = System.Drawing.Color.Black;
                        backGroundColor.Font = new Font(LangMgr.GetFont(), FontStyle.Bold);
                    }
                    if(column.Index==3 || column.Index==4)
                        backGroundColor.TextAlignment = DevAge.Drawing.ContentAlignment.MiddleRight;
                    else
                        backGroundColor.TextAlignment = DevAge.Drawing.ContentAlignment.MiddleLeft;


                    return (Convert.ToInt32(row["RowNumber"])) % 2 != 0;
                }
                catch (Exception ex)
                {
                    MessageBox.Show(ex.Message);
            ...
            gridColumn = dgCashFlow.Columns.Add("S.N", "S.N", new SourceGrid.Cells.DataGrid.Cell());
            gridColumn.HeaderCell.View = viewColumnHeader;
            gridColumn.Width = 50;
            gridColumn.Visible = false;

            gridColumn = dgCashFlow.Columns.Add("Account", "Account Name", new SourceGrid.Cells.DataGrid.Cell());
            gridColumn.HeaderCell.View = viewColumnHeader;
            gridColumn.DataCell.AddController(dblClick);
            gridColumn.Conditions.Add(con_backGroundView);
            ...
            gridColumn = dgCashFlow.Columns.Add("AccountID", "AccountID", new SourceGrid.Cells.DataGrid.Cell());
            gridColumn.HeaderCell.View = viewColumnHeader;
            gridColumn.Width = 100;
            gridColumn.Visible = false;
            gridColumn = dgCashFlow.Columns.Add("AmountInflow", "Amount Inflow", new SourceGrid.Cells.DataGrid.Cell());
            gridColumn.HeaderCell.View = viewColumnHeader;
           gridColumn.DataCell.AddController(dblClick);
           gridColumn.Conditions.Add(con_backGroundView);
            ...
            gridColumn = dgCashFlow.Columns.Add("AmountOutflow", "Amount Outflow", new SourceGrid.Cells.DataGrid.Cell());
            gridColumn.HeaderCell.View = viewColumnHeader;
            gridColumn.DataCell.AddController(dblClick);
            gridColumn.Conditions.Add(con_backGroundView);
            gridColumn.Conditions.Add(con_amountLdg);
            ...
            gridColumn = dgCashFlow.Columns.Add("Type", "Type", new SourceGrid.Cells.DataGrid.Cell());
            gridColumn.HeaderCell.View = viewColumnHeader;
            gridColumn.Width = 100;
            gridColumn.Visible = false;

            dgCashFlow.AutoStretchColumnsToFitWidth = true;
        ...
        private void CreateData()
        {
            ...
            dtGridData = CashFlow.GetCFReportDataForDataGrid(m_CashFlow.FromDate, m_CashFlow.ToDate, "<AccClassIDSettings><AccClassID>1</AccClassID></AccClassIDSettings>", "<ProjectIDSettings><ProjectID>1</ProjectID></ProjectIDSettings>");
            if (dtGridData.Rows.Count > 0)
            {
                ds.Tables.Add(dtGridData);
                ds.Relations.Add("ChildRows", ds.Tables[0].Columns["AccountID"], ds.Tables[0].Columns["GroupID"], false);
                if (m_CashFlow.AccountWise)
                {
                    TotalAmountOut = 0;
                    TotalAmountIn = 0;
                    foreach (DataRow dr in ds.Tables[0].Rows)
                    {
                        ...
                        if (dr["Type"].ToString() == "LEDGER")
                        {
                            TotalAmountIn += Convert.ToDecimal(dr["AmountInFlow"]);
                            TotalAmountOut += Convert.ToDecimal(dr["AmountOutFlow"]);
                                                       
                            dtTemp.Rows.Add("1", dr["Account"].ToString(), dr["AccountID"].ToString(), Convert.ToDecimal(dr["AmountInFlow"]).ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)), Convert.ToDecimal(dr["AmountOutFlow"]).ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)), dr["Type"].ToString(),rowNumber);
                            rowNumber++;
                        }
                    }
                    dtTemp.Rows.Add("-", "TOTAL CASH FLOW", " ", TotalAmountIn.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)), TotalAmountOut.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)), "TOTAL",rowNumber);
                }
                else if (m_CashFlow.GroupWise)
                {
                    TotalAmountIn = 0;
                    TotalAmountOut = 0;
                    foreach (DataRow dr in ds.Tables[0].Rows)
                    {
                        ...
                        if (dr["GroupId"] is DBNull || Convert.ToInt32(dr["GroupID"]) == 0)
                        {                         
                            TotalAmountIn += Convert.ToDecimal(dr["AmountInFlow"]);
                            TotalAmountOut += Convert.ToDecimal(dr["AmountOutFlow"]);
                            dtTemp.Rows.Add("-", dr["Account"].ToString(), dr["AccountID"].ToString(), Convert.ToDecimal(dr["AmountInFlow"]).ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)), Convert.ToDecimal(dr["AmountOutFlow"]).ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)), dr["Type"].ToString(),rowNumber);
                            rowNumber++;
                            FormatChildRows(dr, 1);
                        }
                    }
                    dtTemp.Rows.Add("-", "TOTAL CASH FLOW", " ", TotalAmountIn.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)), TotalAmountOut.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)), "TOTAL",rowNumber);
                }
        ...
        private void FormatChildRows(DataRow dr, int level)
        {
            string space = "            ";
            ...
            DataRow[] Childs = dr.GetChildRows("ChildRows");
            foreach(DataRow ChildRow in Childs)
            {
                //AmountIn = 0;
                //AmountOut = 0;

                //if (Convert.ToDecimal(ChildRow["AmountFlow"]) >= 0)
                //{
                //    AmountIn = Convert.ToDecimal(ChildRow["AmountFlow"]);
                //    AmountOut = 0;
                //}
                //else
                //{
                //    AmountOut = 0 - Convert.ToDecimal(ChildRow["AmountFlow"]);
                //    AmountIn = 0;
                //}

                if(ChildRow["Type"].ToString()=="GROUP")
                {
                    if (m_CashFlow.Details)
                    {
                        dtTemp.Rows.Add("-", space + ChildRow["Account"].ToString(), ChildRow["AccountID"].ToString(), Convert.ToDecimal(ChildRow["AmountInFlow"]).ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)), Convert.ToDecimal(ChildRow["AmountOutFlow"]).ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)), ChildRow["Type"].ToString(),rowNumber);
                        rowNumber++;
                        if (ChildRow.GetChildRows("ChildRows").Length > 0)
                        {
                            level++;
                            FormatChildRows(ChildRow, level);
                            level--;
                        }
                    }
                    else if(m_CashFlow.Summary)
                    {
                        if(m_CashFlow.ShowLedger)
                            FormatChildRows(ChildRow, level);
                    }
                }
                else if(ChildRow["Type"].ToString()=="LEDGER")
                {
                    if (m_CashFlow.ShowLedger)
                    {
                        dtTemp.Rows.Add("1", space + ChildRow["Account"].ToString(), ChildRow["AccountID"].ToString(), Convert.ToDecimal(ChildRow["AmountInFlow"]).ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)), Convert.ToDecimal(ChildRow["AmountOutFlow"]).ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)), ChildRow["Type"].ToString(), rowNumber);
                        rowNumber++;
                    }
                }
            }
        ...
        private void frmCashFlow_Load(object sender, EventArgs e)
        {
           ...
           //DisplayBannar();
            ...
            try
            {
                ...
                dblClick = new SourceGrid.Cells.Controllers.CustomEvents();
                dblClick.DoubleClick += new EventHandler(grdCashFlow_DoulbeClick);
                dblClick.KeyUp += new KeyEventHandler(grdCashFlow_KeyUp);

                grdCashFlow.Visible = false;
                dgCashFlow.Visible = true;
                DisplayBannar();
                FillGridWithData();
        ...
        private void grdCashFlow_KeyUp(object sender, KeyEventArgs e)
        {
        ...
        private void grdCashFlow_DoulbeClick(object sender, EventArgs e)
            ...
            //string Type = (cellType.DisplayText).ToString();
             ...
             int CurRow = dgCashFlow.Selection.GetSelectionRegion().GetRowsIndex()[0];
             SourceGrid.CellContext cellType = new SourceGrid.CellContext(dgCashFlow, new SourceGrid.Position(CurRow, 5));
             string Type = (cellType.DisplayText).ToString();
          
            if (Type == "GROUP")
            {

                //int CurRow1 = dgCashFlow.Selection.GetSelectionRegion().GetRowsIndex()[0];
                //SourceGrid.CellContext cellID = new SourceGrid.CellContext(dgCashFlow, new SourceGrid.Position(CurRow1, 2));
            ...
            {
                SourceGrid.CellContext cellID = new SourceGrid.CellContext(dgCashFlow, new SourceGrid.Position(CurRow, 2));
                  if (cellID.Value != "")
                  {
                      TransactSettings m_TS = new TransactSettings();
                      m_TS.HasDateRange = true;
                      ...
                      m_TS.ProjectID = m_CashFlow.ProjectID;

                      frmTransaction m_Transact = new frmTransaction(m_TS,m_mainMDI);
        ...
        private void btnPrintPreview_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                dsCashFLow.Tables.Remove("tblCashFlow");
            }
            catch
            {
            }
            System.Data.DataView view = new System.Data.DataView(dtTemp);
            DataTable selected = view.ToTable("tblCashFlow", false,"Account","AmountInflow","AmountOutflow","Type");
            dsCashFLow.Tables.Add(selected);
        ...
        private void grdCashFlow_Paint(object sender, PaintEventArgs e)
        {
