...
namespace Accounts
{
    public partial class frmAccountLedgerSettings : Form, IfrmSelectAccClassID,IfrmDateConverter
    {
        ...
        //    InitializeComponent();
        //}
        public frmAccountLedgerSettings(IMDIMainForm frm)
        {
            frmMDI = frm;
            InitializeComponent();
        }
        ...
        private void frmAccountLedgerSettings_Load(object sender, EventArgs e)
        {
            ...
            {
                cmbChooseLedger.Items.Add(new ListItem((int)drLedgerInfo["LedgerID"], drLedgerInfo["EngName"].ToString()));//It adds LedgerID as well as LedgerName in combobox               
            }
 ...
 #region Language Management
  ...
  #region  //For Details
                if (rbtnDetails.Checked == true)
                {
                    ...
                    m_transactSetting.ShowRemarks = chkShowRemarks.Checked;
                    m_transactSetting.ShowZeroBalance = chkZeroBalance.Checked;
                    ...
                    if (rbtnChooseLedger.Checked)
                    {
                        ...
                        {
                            frmTransaction frm = new frmTransaction(m_transactSetting, frmMDI);
                            frm.ShowDialog();
                        }
                        else//This is 
                        {
                            frmTransaction frm = new frmTransaction(m_transactSetting, "ALL_LEDGERS", frmMDI);
                            frm.ShowDialog();
                        }
                    }
                    ...
                    if (rbtnChooseAccountGrp.Checked)
                    {
                        ...
                        {
                            frmTransaction frm = new frmTransaction(m_transactSetting, "PARTICULAR_GROUP",frmMDI);
                            frm.ShowDialog();
                        }
                        else
                        {
 ...
 #region For summary
                ...
                else if (rbtnSummary.Checked == true)
                {
                    ...
                    TransactSettings m_transactSettings = new TransactSettings();
                    m_transactSettings.HasDateRange = chkDate.Checked;
                    if (chkDate.Checked)
                    {
                        m_transactSettings.FromDate = Date.ToDotNet(txtFromDate.Text);
                        m_transactSettings.ToDate = Date.ToDotNet(txtToDate.Text);
                    }
                    else
                    {
                        m_transactSettings.FromDate = null;
                        m_transactSettings.ToDate = null;
                    }
                    ...
                    m_transactSettings.ShowRemarks = chkShowRemarks.Checked;
                    m_transactSettings.ShowZeroBalance = chkZeroBalance.Checked;

                    if (rbtnChooseLedger.Checked)
                    {
                        if (cmbChooseLedger.SelectedIndex != 0)
                        {
                            frmSTransaction frm = new frmSTransaction(m_transactSettings, frmMDI);
                            frm.ShowDialog(); 
                        }
                        else 
                        {
                            frmSTransaction frm = new frmSTransaction(m_transactSettings, "ALL_LEDGERS", frmMDI);
                            frm.ShowDialog();
                        }
                    }
                    ...
                    if (rbtnChooseAccountGrp.Checked)
                    {
                        if (cmbChooseAccountGroup.SelectedIndex != 0)
                        {
                            frmSTransaction frm = new frmSTransaction(m_transactSettings, "PARTICULAR_GROUP", frmMDI);
                            frm.ShowDialog();
                        }
                        else
                        {
        ...
        {
            //just for test
            BudgetReportSetting BS = new BudgetReportSetting();
            try
            {
                BS.AccClassID = AccClassID;
            }
            catch
            {
                //Ignore 
            }
            frmSelectAccClass frm = new frmSelectAccClass(this, BS.AccClassID);
        ...
        private void cmbChooseLedger_SelectedIndexChanged(object sender, EventArgs e)
        {
