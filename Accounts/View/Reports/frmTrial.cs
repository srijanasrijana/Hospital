...
namespace Accounts.Reports
{
    public partial class frmTrial : Form
    {
        ...
        private Accounts.Model.dsTrailBalancePreviousYear dsTrialPY = new Model.dsTrailBalancePreviousYear();
        ...
        public frmTrial(TrialBalanceSettings TBS, Form accesscontrol, IMDIMainForm frm)//Constructor having class object as argument
        {
            m_MDIForm = frm;
        ...
        private void CreateColumnsforTempTable()
        {
            ...
            dtTemp.Columns.Add("SNo", typeof(string));//column for ledger Code ;just to match the name of dataset for crystal report
            dtTemp.Columns.Add("Name", typeof(string));
            dtTemp.Columns.Add("LedgerID", typeof(string));
            dtTemp.Columns.Add("Type", typeof(string));

            dtTemp.Columns.Add("Debit_Amount", typeof(string));
            dtTemp.Columns.Add("Credit_Amount", typeof(string));
        }
        DataTable dtTrialGridData = new DataTable();
        decimal TotalDebitAmount = 0;
        decimal TotalCreditAmount = 0;
        decimal opBalDr = 0;
        decimal opBalCr = 0;
        decimal prevYearPnL = 0;
        private void FillGridWithData(decimal ProductValue1)
        {

            prevYearPnL = Convert.ToDecimal(Settings.GetSettings("PREV_YR_PROFIT"));
           
            dtTrialGridData.Clear();
            dgTrial.Visible = true;
            grdTrial.Visible = false;

            decimal trialOpenDiff = 0;
            try
            {
                CreateColumnsforTempTable(); //create column head for temp table
                if (ProductValue1 > 0)
                {
                    dtTemp.Rows.Add("-", " ", "Opening Stock", " ", " ", ProductValue1.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)), 0.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces))); //add first row as opening balance
                }
                else
                {
                    if (m_TBS.ShowZeroBalance == true)
                        dtTemp.Rows.Add("-", " ", "Opening Stock", " ", " ", ProductValue1.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)), 0.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces))); //add first row as opening balance
                }
                ...
                TotalDebitAmount += ProductValue1;
                DataTable dtGroup = new DataTable();
                //if (m_TBS.OnlyPrimaryGroups == true)
                //{
                //    dtGroup = Ledger.GetPrimaryGroupIDnName();
                //}
                if (m_TBS.AllGroups == true || m_TBS.OnlyPrimaryGroups == true || m_TBS.LedgerOnly == true)
                {
                    //dtGroup = Ledger.GetGroupIDnNameFromtblLedger();
                    DataSet ds = new DataSet();
                    DataTable dtAllGroup = new DataTable("dtAllGroup");
                    //double DrBal, CrBal;
                    //DrBal = CrBal = 0;
                    decimal groupBal = 0;
                    dtAllGroup = Ledger.GetAllGroupNLedgerBal("<AccClassIDSettings><AccClassID>1</AccClassID></AccClassIDSettings>", "<ProjectIDSettings><ProjectID>1</ProjectID></ProjectIDSettings>", null, m_TBS.ToDate, 0, null);
                    ds.Tables.Add(dtAllGroup);
                    ds.Relations.Add("ChildRows", ds.Tables[0].Columns["AccountID"], ds.Tables[0].Columns["ParentGroupID"], false);

                    foreach (DataRow dr in ds.Tables[0].Rows)
                    {
                        //DrBal = CrBal = 0;
                        if (Convert.ToInt32(dr["ParentGroupID"]) == 0)
                        {
                            if (m_TBS.LedgerOnly == false)
                            {
                                opBalDr += Convert.ToDecimal(dr["OpenBalDr"]);
                                opBalCr += Convert.ToDecimal(dr["OpenBalCr"]);
                                if (Global.isOpeningTrial == false)
                                    groupBal = (Convert.ToDecimal(dr["DebitTotal"]) + Convert.ToDecimal(dr["OpenBalDr"])) - (Convert.ToDecimal(dr["CreditTotal"]) + Convert.ToDecimal(dr["OpenBalCr"]));
                                else
                                    groupBal = Convert.ToDecimal(dr["OpenBalDr"]) - Convert.ToDecimal(dr["OpenBalCr"]);
                                //DrBal = Convert.ToDouble(dr["DebitTotal"]) + Convert.ToDouble(dr["OpenBalDr"]);
                                //CrBal = Convert.ToDouble(dr["CreditTotal"]) + Convert.ToDouble(dr["OpenBalCr"]);
                                //TotalDebitAmount += DrBal;
                                //TotalCreditAmount += CrBal
                                if (groupBal >= 0)
                                {
                                    dtTemp.Rows.Add("-", dr["AccountCode"].ToString(), dr["AccountName"].ToString(), dr["AccountID"].ToString(), "GROUP", groupBal.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)), 0.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)));
                                    TotalDebitAmount += groupBal;
                                }
                                else
                                {
                                    dtTemp.Rows.Add("-", dr["AccountCode"].ToString(), dr["AccountName"].ToString(), dr["AccountID"].ToString(), "GROUP", 0.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)), (0 - groupBal).ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)));
                                    TotalCreditAmount += (0 - groupBal);
                                }
                            }
                            FillTempForChild(dr, 1);
                        }
                    }
                }
                if (prevYearPnL >= 0)
                {
                    dtTemp.Rows.Add("-", " ", "Previous Year P/L(Profit)", " ", " ", 0.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)), prevYearPnL.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)));
                }
                else
                {
                    dtTemp.Rows.Add("-", " ", "Previous Year P/L(Loss)", " ", " ", 0.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)),"(" +(0-prevYearPnL).ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces))+")");

                }
                TotalCreditAmount += prevYearPnL;
                #region Calculating Defference in opening balance and trial total
                trialOpenDiff = (opBalDr + ProductValue1) - (opBalCr + prevYearPnL); //find opening trial balance difference
                if (trialOpenDiff >= 0) //add difference value on credit side to balance
                {
                    dtTemp.Rows.Add("-", " ", "DIFFERENCE IN OPENING BALANCE", " ", " ", 0.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)), trialOpenDiff.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)));
                    TotalCreditAmount += trialOpenDiff;
                }
                else
                {
                    dtTemp.Rows.Add("-", " ", "DIFFERENCE IN OPENING BALANCE", " ", " ", (Math.Abs(trialOpenDiff)).ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)), 0.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)));
                    TotalDebitAmount += Math.Abs(trialOpenDiff);
                }
                dtTemp.Rows.Add("-", " ", "TRIAL TOTAL", " ", " ", TotalDebitAmount.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)), TotalCreditAmount.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)));
                ...
                dgTrial.Columns.Clear(); // first clear all columns to reload the data in dgDayBook
                dgTrial.FixedRows = 1;
                DevAge.ComponentModel.IBoundList bd = new DevAge.ComponentModel.BoundDataView(mView);

                CreateDataGridColumns(dgTrial.Columns, bd);
                dgTrial.DataSource = bd;
                dgTrial.Width = dgTrial.Width - 5;
                dgTrial.Width = this.Width - 25;
                #endregion
            }
            catch (Exception ex)
            {
        ...
        private void FillTempForChild(DataRow dr, int level)
        {
            string space = "            ";
            ...
            foreach (DataRow childrow in ChildRows)
            {
                ...
                if (childrow["AccountType"].ToString() == "GROUP")
                {
                    ...
                    {
                        if (Global.isOpeningTrial == false)
                            groupBal = (Convert.ToDecimal(childrow["DebitTotal"]) + Convert.ToDecimal(childrow["OpenBalDr"])) - (Convert.ToDecimal(childrow["CreditTotal"]) + Convert.ToDecimal(childrow["OpenBalCr"]));
                        else
                            groupBal = Convert.ToDecimal(childrow["OpenBalDr"]) - Convert.ToDecimal(childrow["OpenBalCr"]);
                        if (groupBal > 0)
                        {
                            dtTemp.Rows.Add("-", childrow["AccountCode"].ToString(), space.ToString() + childrow["AccountName"].ToString(), childrow["AccountID"].ToString(), "GROUP", groupBal.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)), 0.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)));
                        }
                        else if (groupBal < 0)
                        {
                            dtTemp.Rows.Add("-", childrow["AccountCode"].ToString(), space.ToString() + childrow["AccountName"].ToString(), childrow["AccountID"].ToString(), "GROUP", 0.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)), (0 - groupBal).ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)));
                        }
                        else
                        {
                            if (m_TBS.ShowZeroBalance)
                            {
                                dtTemp.Rows.Add("-", childrow["AccountCode"].ToString(), space.ToString() + childrow["AccountName"].ToString(), childrow["AccountID"].ToString(), "GROUP", 0.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)), 0.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)));
                            }
                            else
                            {
                                continue;
                        ...
                        {
                            level++;
                            FillTempForChild(childrow, level);
                            level--;
                        }
                    }
                    else
                ...
                else if (childrow["AccountType"].ToString() == "LEDGER")
                {
                    if (m_TBS.LedgerOnly == true)
                    {
                        ...
                        if (groupBal > 0)
                        {
                            dtTemp.Rows.Add("1", childrow["AccountCode"].ToString(), space.ToString() + childrow["AccountName"].ToString(), childrow["AccountID"].ToString(), "LEDGER", groupBal.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)), 0.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)));
                            TotalDebitAmount += groupBal;
                        }
                        else if (groupBal < 0)
                        {
                            dtTemp.Rows.Add("1", childrow["AccountCode"].ToString(), space.ToString() + childrow["AccountName"].ToString(), childrow["AccountID"].ToString(), "LEDGER", 0.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)), (0 - groupBal).ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)));
                            TotalCreditAmount += (0 - groupBal);
                        ...
                        else
                        {
                            if (m_TBS.ShowZeroBalance)
                            {
                                dtTemp.Rows.Add("1", childrow["AccountCode"].ToString(), space.ToString() + childrow["AccountName"].ToString(), childrow["AccountID"].ToString(), "LEDGER", 0.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)), 0.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)));
                            }

                        }
                        opBalDr += Convert.ToDecimal(childrow["OpenBalDr"]);
                        opBalCr += Convert.ToDecimal(childrow["OpenBalCr"]);
                    ...
                    else
                    {
                        if (m_TBS.Details == true)
                        {
                            ...
                            if (Global.isOpeningTrial == false)
                            {
                                currentDr = Convert.ToDecimal(childrow["DebitTotal"]) + Convert.ToDecimal(childrow["OpenBalDr"]);
                                currentCr = Convert.ToDecimal(childrow["CreditTotal"]) + Convert.ToDecimal(childrow["OpenBalcr"]);
                            }
                            else
                            {
                                currentDr = Convert.ToDecimal(childrow["OpenBalDr"]);
                                currentCr = Convert.ToDecimal(childrow["OpenBalcr"]);
                            }
                            if (currentDr - currentCr == 0)
                            {
                                if (m_TBS.ShowZeroBalance)
                                    dtTemp.Rows.Add("1", childrow["AccountCode"].ToString(), space.ToString() + childrow["AccountName"].ToString(), childrow["AccountID"].ToString(), "LEDGER", 0.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)), 0.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)));
                            }
                            ...
                            {
                                dtTemp.Rows.Add("1", childrow["AccountCode"].ToString(), space.ToString() + childrow["AccountName"].ToString(), childrow["AccountID"].ToString(), "LEDGER", currentDr.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)), currentCr.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces)));
                            }
                        }
                    }
                }
        ...
        private void CreateDataGridColumns(SourceGrid.DataGridColumns columns, DevAge.ComponentModel.IBoundList boundList)
        {
           ...
           //cellView.Font = new Font("Arial", 9, FontStyle.Bold);
           // boldView.BackColor = Color.DarkSeaGreen;
            ...
            //SourceGrid.Cells.Views.Cell Colorview = new SourceGrid.Cells.Views.Cell();
            //Colorview.BackColor = Color.DarkSeaGreen;
            ...
            gridColumn = dgTrial.Columns.Add("S.N", "S.N", new SourceGrid.Cells.DataGrid.Cell());
            gridColumn.HeaderCell.View = viewColumnHeader;
            gridColumn.Width = 50;
            gridColumn.Visible = false;

            gridColumn = dgTrial.Columns.Add("SNo", "Account Code", new SourceGrid.Cells.DataGrid.Cell());
            gridColumn.HeaderCell.View = viewColumnHeader;
            ...
            gridColumn = dgTrial.Columns.Add("Name", "Account Name", new SourceGrid.Cells.DataGrid.Cell());
            gridColumn.HeaderCell.View = viewColumnHeader;
            gridColumn.DataCell.AddController(dblClick);
            gridColumn.Conditions.Add(con_LdgView);
            gridColumn.Conditions.Add(con_BoldView);
            ...
            gridColumn = dgTrial.Columns.Add("LedgerID", "LedgerID", new SourceGrid.Cells.DataGrid.Cell());
            gridColumn.HeaderCell.View = viewColumnHeader;
            gridColumn.Width = 100;
            gridColumn.Visible = false;

            gridColumn = dgTrial.Columns.Add("Type", "Type", new SourceGrid.Cells.DataGrid.Cell());
            gridColumn.HeaderCell.View = viewColumnHeader;
            gridColumn.Width = 100;
            gridColumn.Visible = false;


            gridColumn = dgTrial.Columns.Add("Debit_Amount", "DebitAmount", new SourceGrid.Cells.DataGrid.Cell());
            gridColumn.HeaderCell.View = viewColumnHeader;
            gridColumn.DataCell.AddController(dblClick);
            ...
            gridColumn = dgTrial.Columns.Add("Credit_Amount", "CreditAmount", new SourceGrid.Cells.DataGrid.Cell());
            gridColumn.HeaderCell.View = viewColumnHeader;
            gridColumn.DataCell.AddController(dblClick);
            gridColumn.Conditions.Add(con_amountLdg);
            gridColumn.Conditions.Add(con_amountBold);
            ...
            dgTrial.AutoStretchColumnsToFitWidth = true;
        ...
        private void DisplayTrialBlance(bool IsCrystalReport)
        {
            ...
            if (!IsCrystalReport)//Need only for writting on grid...not for crystal reports
            {
                ...
                dgTrial.SelectionMode = SourceGrid.GridSelectionMode.Row;
                dgTrial.Selection.EnableMultiSelection = false;

                dblClick = new SourceGrid.Cells.Controllers.CustomEvents();
                dblClick.DoubleClick += new EventHandler(grdTrial_DoubleClick);
                dblClick.KeyUp += new KeyEventHandler(dgTrialBalance_keyUp);
            ...
            foreach (DataRow drProduct in dtGetAllProduct.Rows)
            {
                ...
                if (dtOpeningStockStatusInfo1.Rows.Count > 0)
                {
                    ...
                    decimal ProdPrice =Convert.ToDecimal( Product.GetProductPurchasePrice(Convert.ToInt32(drProduct["ProductID"].ToString()), Global.ParentAccClassID));
                    ProductValue1 += ProdPrice * closingQuantity1;
                }
            }
            //new change
            FillGridWithData(ProductValue1);
            #region Old code for only ledger
            ...
            #region Old code for groups
            //#region BLOCK FOR ONLY PRIMARY GROUPS
            //if (m_TBS.OnlyPrimaryGroups == false)
            //{
            ...
            #region old code of BLOCK FOR CALCULATING DIFFERENCE IN OPENING BALANCE
        ...
        private void frmTrial_Load(object sender, EventArgs e)
        {
            ...
            AccClassIDsXMLString = GetXMLAccClass(m_TBS.AccClassID);

            ArrayList arrProject = new ArrayList();
            arrProject.Add(m_TBS.ProjectID);
            string ProjectIDsXMLString = Project.GetXMLProject(arrProject);

            DisplayTrialBlance(false);
            dgTrial.EnableSort = false;
            this.WindowState = FormWindowState.Minimized;
            this.WindowState = FormWindowState.Maximized;

            ////if (Global.isOpeningTrial == false) //Show closing trial
            ////{
            ////    DisplayTrialBlance(false);
            ////}
            ////else //Show only Opening Trial 
            ////{
        ...
        public string GetXMLAccClass(ArrayList arrAccountingClass)
        {
        ...
        private void dgTrialBalance_keyUp(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
                grdTrial_DoubleClick(sender, null);
        }
        ...
        private void grdTrial_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                ...
                int CurRow = dgTrial.Selection.GetSelectionRegion().GetRowsIndex()[0];
                SourceGrid.CellContext cellType = new SourceGrid.CellContext(dgTrial, new SourceGrid.Position(CurRow, 4));
                SourceGrid.CellContext cellTypeID = new SourceGrid.CellContext(dgTrial, new SourceGrid.Position(CurRow, 3));
                ...
                if (Type == "GROUP")
                {
                    int CurRow1 = dgTrial.Selection.GetSelectionRegion().GetRowsIndex()[0];
                    SourceGrid.CellContext cellID = new SourceGrid.CellContext(dgTrial, new SourceGrid.Position(CurRow1, 3));
                    ...
                    m_GBS.ProjectID = m_TBS.ProjectID;
                    ...
                    frmGroupBalance m_GrpBal = new frmGroupBalance(m_GBS, m_MDIForm);
                    m_GrpBal.ShowDialog();
                }
                else if (Type == "LEDGER")
                {
                    int CurRow2 = dgTrial.Selection.GetSelectionRegion().GetRowsIndex()[0];
                    SourceGrid.CellContext cellID = new SourceGrid.CellContext(dgTrial, new SourceGrid.Position(CurRow2, 3));
                    ...
                    m_TS.ProjectID = m_TBS.ProjectID;
        ...
        private void PrintPreviewCR(PrintType myPrintType)
        {
            ...
            if (m_TBS.ShowPreviousYear == false)
            {
                ...
                try
                {
                    dsTrial.Tables.Remove("tblGroup");
                }
                catch
                {
                }
                ...
                try
                {
                    DataTable selected = view.ToTable("tblGroup", false, "SNo", "Name", "Debit_Amount", "Credit_Amount", "Type");
                    dsTrial.Tables.Add(selected);
                }
                catch (Exception ex)
                {
                    Global.MsgError(ex.Message);
            ...
            if (m_TBS.ShowPreviousYear == false)
            {
                ...
                pdvDebit_Total.Value = TotalDebitAmount.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces));
                pvCollection.Clear();
                pvCollection.Add(pdvDebit_Total);
                rpt.DataDefinition.ParameterFields["Debit_Total"].ApplyCurrentValues(pvCollection);

                pdvCredit_Total.Value = TotalCreditAmount.ToString(Misc.FormatNumber(Global.Comma_Separated, Global.DecimalPlaces));
                pvCollection.Clear();
        ...
        #endregion
        private void dgTrial_Paint(object sender, PaintEventArgs e)
        {
