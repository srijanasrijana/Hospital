...
namespace Accounts
{
    ...
    {
        void AddOpeningBalance(DataTable AllOpeningBalance,DateTime? DueDate);
    }

    public partial class frmOpeningBalance : Form, IDueDate
    {       
        ...
        public frmOpeningBalance(Form ParentForm, OpeningBalanceSettings OBS)
        {
           ...
           // this.Font = LangMgr.GetFont();
            ...
            this.LedgerName = OBS.LedgerName;
        ...
        private void btnGrpSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (grdOpeningBalnce[1, 3].ToString() == "Debit")
                {
                    DueDate();
                    frmDueDate frm = new frmDueDate(this, dt1,0,"OPBAL");
                    if (dt1.Rows.Count > 0)
                    {
                        frm.ShowDialog();
                    }
                ...
                if(dtDueDateInfo.Rows.Count>0)
                {
                    dueDate = Convert.ToDateTime(dtDueDateInfo.Rows[0][0].ToString());
                }


                m_ParentForm.AddOpeningBalance(dt, dueDate);
                this.Hide();
            }
            catch (Exception ex)
            {
        ...
        private void DueDate()
        {
            try
            {
                ...
                //tw.WriteElementString("LedgerID", LedgerID.ToString());//check wheather ledgerid is with in debtor group or not
                ...
                //tw.Close();
                //string strXML = "";
                dttemp = Ledger.FilterLedgersByGroup("", ParentGroupID, 29);
                    if(dttemp.Rows.Count>0)
                    {
                        dt1.Rows.Add(LedgerID, LedgerName);
                    }
            }
            catch (Exception ex)
            {
